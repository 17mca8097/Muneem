...
public class MorningMilk extends Fragment {

    android.widget.DatePicker datePicker;
    Button btnShowList;
    private RecyclerView MorningMilkRecyclerView;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_morning_milk, container, false);
        globals = GlobalVariables.getGlobalInstance();
        MorningMilkRecyclerView = view.findViewById(R.id.MorningMilkList);
    ...
    private void BuildMMRList(String date) {
        ...
        listMMR = dbh.GetSavedMorningMilkRecord(date,1);

        if (listMMR != null) {
            adapterMMR = new AdapterMMR(listMMR);
            MorningMilkRecyclerView.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
            MorningMilkRecyclerView.setAdapter(adapterMMR);
        } else {
            Toast.makeText(getContext(), "No Morning-Milk-Record Found", Toast.LENGTH_SHORT).show();
        }
